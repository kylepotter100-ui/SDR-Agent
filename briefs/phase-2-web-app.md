...
- **Full re-personalisation under v5 — HARD PRE-LAUNCH GATE.** The DB still holds pre-v5 personalisations (product-led copy) and pre-v5 `ranking_score`s on prospects not touched by the realignment spot-check. Before any live send, clear ALL personalisations and re-personalise the whole pool under the v5 prompt, then run a full no-limit `rank()` so every prospect carries a v5 score (`ranking_score` is `numeric` — sort is correct; the issue is purely stale values, not an ordering bug). Sequence: clear personalised_email_subject/body across all prospects → re-run personalise → re-run rank (no limit). Unblocked by the batching fix below — the full re-rank now fits the function ceiling.
- ~~**Rank batching — PRE-LAUNCH BLOCKER, not optional.**~~ **Resolved.** `rank()` now partitions the full unsurfaced pool into chunks of 56 and fires them as independent concurrent Opus calls via a Promise.allSettled runner (`MAX_CONCURRENCY = 6`), so wall-clock collapses to ≈ max(chunk) instead of the sum. Effort stays at `"medium"` — no quality trade. One bad chunk no longer nukes the run; rejected chunks bucket into `errors.byBucket` and those prospects retry on the next run. Empirical: Step 0 measured a 56-chunk at 107s on the main preview, predicting ~190s margin under the 300s ceiling for the full ~167. PR proof is the full no-limit timed run.
- **Ranker reasoning micro-pass (minor, non-blocking).** A couple of v5 ranker reasoning lines still phrase fit as "booking system fit" — internal reasoning only, never prospect-facing, but inconsistent with the partnership/outcome positioning. Tighten toward outcome/partnership language on a convenient later pass; not worth a dedicated PR.
