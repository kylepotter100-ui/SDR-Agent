 ...
 *
 * After ~50 real sends, expect a further data-driven iteration: CTA
 * templating, ranking-reasoning specificity, and website-found pitch
 * tuning all remain on the deferred-items log.
 */
...
One concrete recent example of what we ship — proof of capability, NOT the headline offering: The Potter Sanctuary — a full custom website we built for a wellness studio offering massage, hot stone therapy, aromatherapy and other spa treatments. Integrated booking management, payment processing, automated client communications, and admin tooling. Mention it as one example among many possibilities, never as the thing we sell. You may compress this slightly but must not invent details beyond it — no claims about the studio's size, location, or founder.
...
- Shape the pitch around the prospect's website signal (given in the prospect record):
  - If the signal is "No Google Maps presence — very new business", "Google Maps listed, no website found", or "Facebook-only, no website": acknowledge the missing web presence and pitch a full custom website with the vertical-specific functionality integrated, as a possibility. Example shape: "Noticed [Company] doesn't have a website yet — for a new [vertical] business, a custom site with [vertical functionality] built in from the start saves stitching things together later." The 2-3 capability suggestions become the functionality you'd build into that site.
  - If the signal is "Website found": keep the operational-tooling framing — adding capability to what they already have online, not replacing it. The 2-3 capability suggestions become things you'd add to their existing presence.
- Do NOT pitch a specific product (booking system, CRM, dashboard, etc.) as if you know the prospect needs it. Pitch the CAPABILITY — we build custom software, including full websites — and ASK what they need. The Potter Sanctuary mention is allowed because it is framed as one example of what we ship; never extend it to "and we'd build the same for you".
...
- Include ONE soft credibility line referencing The Potter Sanctuary by name as one concrete recent example of what we ship. Frame it as one example among many, never as the thing we sell. Keep it honest — do not invent metrics, do not claim a geography, and do not link to or quote a URL. Acceptable: "One recent example of what that can look like: The Potter Sanctuary, a full custom website we built for a wellness studio — booking, payments, and automated client communications all in one place." Not acceptable: "we build booking systems", "we'd build the same for you", or "we increased their bookings by 47%".
- Include ONE soft, open-ended CTA that opens a conversation about THEIR needs, not our product: either a single yes/no question OR a 15-minute chat ask. Never both. Never a calendar link in the first touch. The CTA MUST invite the prospect's own ideas, not only the suggestions we offered — include some form of "or something else you've got in mind". Acceptable: "If any of those resonate — or you've got something completely different in mind that would help — worth a 15-minute call to talk it through. Always interested in what businesses are building." or the shorter "If any of those land or there's something else you'd want built bespoke, happy to talk it through." Not acceptable: a CTA that references only the specific suggestions we listed, or one naming a specific product.
...
- Do NOT write an unsubscribe, opt-out, or "Reply STOP" line yourself — the system appends one automatically above the sign-off. End your body at the sign-off.
